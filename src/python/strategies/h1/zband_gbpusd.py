"""zband_gbpusd.py — ZBandGBPUSDH1

GBPUSD · H1 · BOTH · Z-Band Mean Reversion (Ornstein-Uhlenbeck, không cắt lỗ theo giá)

┌─ QUY TẮC VÀO LỆNH ────────────────────────────────────────────────────────────┐
│  VÀO   (đủ CẢ)                                                                │
│     a. |z| > 3.0                                                              │
│     b. nến TRƯỚC cũng ngoài dải: |z(t−1)| > 3.0                               │
│     c. z < −3.0 → MUA · z > +3.0 → BÁN                                        │
│     → khớp tại giá MỞ CỬA nến kế tiếp sau nến tín hiệu                        │
│                                                                               │
│  THOÁT                                                                        │
│     · z về 0                                                                  │
│     · time-stop 192 nến H1                                                    │
...
KẾT QUẢ   Sharpe 0.658 ALL · 0.651 FORM · 0.681 OOS · net 14.47 bps/lệnh (t = 2.04) ·
          thắng 68.8% · 6/7 năm dương
TẦN SUẤT  112 lệnh trong 6.5 năm ≈ 17 lệnh/năm · giữ 192 nến H1
CHỈ BÁO   z = (log giá − trung bình 96 nến) / độ lệch chuẩn 96 nến · time-stop = 192 nến
          H1
...
EXPECTANCY = (
    "Sharpe 0.658 ALL · 0.651 FORM · 0.681 OOS · net 14.47 bps/lệnh (t = 2.04) · thắng "
    "68.8% · 6/7 năm dương"
)
FREQUENCY = "112 lệnh trong 6.5 năm ≈ 17 lệnh/năm"
...
def _load(start: str = "2020-01-01", broker_markup_pct: float = 1.0):
    ...
    # MAJOR, không phải cross tổng hợp — nạp bằng `load_majors`. Dùng nhầm
    # `load_crosses` thì không tìm thấy symbol và chiến lược chết ngay lúc nạp.
    from research.fx.trade_lab import load_majors
    for ins in load_majors(TIMEFRAME, start=start,
