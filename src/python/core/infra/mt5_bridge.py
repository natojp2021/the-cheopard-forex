...
def _record_order_latency_metric(start_time: float, outcome: str) -> None:
    """Ghi nhận độ trễ (latency) thực tế của order_send_api() thông qua
    observability.MetricsEngine. Thao tác này hoàn toàn độc lập và không bao giờ 
    ảnh hưởng đến luồng điều khiển (bỏ qua mọi exception nếu có)."""
    try:
        from src.python.core.observability import MetricsEngine
        latency_ms = (time.monotonic() - start_time) * 1000.0
        MetricsEngine.record_metric("EXECUTION", "order_send_latency_ms", latency_ms,
                                     warn_thresh=1000.0, crit_thresh=3000.0, unit="ms",
                                     labels={"outcome": outcome})
    except Exception as e:
        log_error(f"Lỗi ghi metric order_send_latency_ms: {e}")
...
def order_send_api(request: Dict[str, Any], max_retries: int = 3) -> Optional[Any]:
    ...
    # Đo ĐỘ TRỄ THẬT của đường gửi lệnh tới broker. Đặt mốc ở ĐÂY chứ không sớm
    # hơn: mọi cổng kiểm tra phía trên đều chạy trong bộ nhớ và mất micro-giây, còn
    # phần thật sự chậm là chặng mạng tới broker. Tính cả phần trên vào sẽ làm loãng
    # con số và không còn phát hiện được lúc broker bắt đầu phản hồi chậm.
    _order_send_t0 = time.monotonic()
    ...
    for attempt in range(1, max_retries + 1):
        ...
        if result.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            _record_order_latency_metric(_order_send_t0, "success")
            return result
        ...
        if not retriable:
            ...
            _record_order_latency_metric(_order_send_t0, "fatal_rejected")
            return result
        ...
        if attempt < max_retries:
            ...
            _record_order_latency_metric(_order_send_t0, "exhausted_retries")
            return result
    return None
