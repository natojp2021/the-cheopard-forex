...
def circuit_breaker_open(*, blocked_symbols: list, why: str) -> bool:
    """Một thư DUY NHẤT cho cả đợt cầu dao mở — KHÔNG một thư trên mỗi công cụ.

    SỰ CỐ 24/08/2026
    =================
    `_email()` từng gọi `order_rejected()` cho TỪNG hành động bị NGẮT MẠCH chặn
    trước khi chạm broker — một cầu dao mở do MỘT nguyên nhân gốc (vd: AutoTrading
    bị tắt tay) cascade thành 22 thư "LỆNH BỊ TỪ CHỐI ... retcode 0" trong một chu
    kỳ. Hai lỗi cùng lúc: retcode 0 không phải mã MT5 thật (lệnh chưa từng rời tiến
    trình để có retcode), và nội dung thư khẳng định "Broker từ chối" trong khi
    broker chưa từng thấy lệnh. Gộp lại còn phản ánh đúng bản chất: một chu kỳ
    trùng breaker-open thì các công cụ đứng chờ đều bị chặn CÙNG một lý do, có sinh
    ra bao nhiêu email khác nhau cũng không thêm được thông tin nào.
    """
    n = len(blocked_symbols)
    symbols_str = ", ".join(blocked_symbols)
    subject = f"🛑 [{_bot()}] CIRCUIT BREAKER ĐANG MỞ — {n} công cụ bị chặn"
    html = (
        "<div style='font-family:Segoe UI,Arial,sans-serif;border:1px solid #dc3545;"
        "border-radius:8px;padding:16px;max-width:600px;background:#ffffff'>"
        "<h3 style='color:#b02a37;margin:0 0 10px 0'>🛑 CIRCUIT BREAKER ĐANG MỞ</h3>"
        f"<div style='font-size:14px;color:#334155;line-height:1.6'>{n} lệnh "
        "KHÔNG được gửi tới broker vì cầu dao đang mở — đây là MỘT sự cố gốc, "
        f"không phải {n} lỗi riêng biệt.</div>"
        + _table([("Tài khoản (Account ID)", os.environ.get("MT5_LOGIN", "Unknown")),
                  ("Công cụ bị chặn", symbols_str),
                  ("Lý do cầu dao mở", why or "—"),
                  ("Build version", _version()),
                  ("Thời điểm", f"{_now_local()} (GMT +7)")])
        + "<p style='font-size:12px;color:#6c757d;border-top:1px solid #dee2e6;"
          f"padding-top:8px;margin-bottom:0'>Cần người vận hành xử lý nguyên nhân "
          f"gốc rồi cầu dao mới tự đóng lại. Email tự động từ {_bot()}.</p>"
        + "</div>"
    )
    text = (
        f"Cảnh báo hệ thống {_bot()}:\n\n"
        f"🛑 CIRCUIT BREAKER ĐANG MỞ — {n} công cụ KHÔNG được gửi lệnh: {symbols_str}\n"
        f"💼 Tài khoản: {os.environ.get('MT5_LOGIN', 'Unknown')}\n"
        f"🔎 Lý do: {why or '—'}\n"
        f"📦 Build: {_version()}\n"
        f"Thời điểm: {_now_local()} (GMT +7)\n"
